...
impl SemanticAnalyzer {
    ...
    pub(super) fn types_match(&self, expected: &TypeAnnot, actual: &TypeAnnot) -> bool {
        ...
        if let (TypeAnnot::Class(a), TypeAnnot::Function { .. }) = (expected, actual) {
            return a == "function";
        }
        if let (TypeAnnot::Function { .. }, TypeAnnot::Class(a)) = (expected, actual) {
            return a == "function";
        }
        if let (TypeAnnot::Function { param_types: pt1, return_type: rt1 },
                 TypeAnnot::Function { param_types: pt2, return_type: rt2 }) = (expected, actual) {
            if pt1.len() != pt2.len() { return false; }
            return pt1.iter().zip(pt2).all(|(a, b)| self.types_match(a, b))
                && self.types_match(rt1, rt2);
        }
        ...
        match expr {
            ...
            Expr::Lambda { params, return_type, body, .. } => {
                ...
                let body_ty = match body {
                    LambdaBody::Expr(e) => self.infer_expr_type(e),
                    LambdaBody::Block(stmts) => {
                        let mut last_ty = TypeAnnot::Void;
                        for s in stmts {
                            self.check_stmt(s);
                            if let Stmt::Return { value: Some(v), .. } = s {
                                last_ty = self.infer_expr_type(v);
                            }
                        }
                        last_ty
                    }
                ...
                self.exit_scope();
                if let Some(ann_ret) = return_type {
                    if !self.types_match(ann_ret, &body_ty) {
                        self.errors.push(self.err(
                            0, 0,
                            format!("Bhai lambda ka return type match nahi karta! Expected {:?} but got {:?}", ann_ret, body_ty),
                        ));
                    }
                }
                TypeAnnot::Function {
            ...
            Expr::ClosureCall { callee, args, line, col } => {
                let callee_ty = self.infer_expr_type(callee);
                for arg in args {
                    self.infer_expr_type(arg);
                }
                if let TypeAnnot::Function { return_type, .. } = &callee_ty {
                    *return_type.clone()
                } else {
                    self.errors.push(self.err(
                        *line, *col,
                        "Bhai closure call kar rahe ho lekin ye function type nahi hai!".to_string(),
                    ));
                    TypeAnnot::Int
                }
            }
