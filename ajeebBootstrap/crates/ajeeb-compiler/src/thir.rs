...
impl ThirChecker {
    ...
    fn check_expr(&mut self, expr: &HirExpr) {
        match expr {
            ...
            HirExpr::Closure { params, body, captures, .. } => {
                ...
                for cap in captures {
                    if let Some(ty) = old_env.get(cap) {
                        self.type_env.insert(cap.clone(), ty.clone());
                    } else {
                        self.errors.push(format!(
                            "Captured variable '{}' ka type pata nahi chal raha!", cap
                        ));
                    }
                }
            ...
            HirExpr::ClosureCall { callee, args, .. } => {
                ...
                if let HirType::Fn { params, .. } = callee.ty() {
                    if params.len() != args.len() {
                        self.errors.push(format!(
                            "Closure call me wrong number of args: expected {}, got {}",
                            params.len(), args.len()
                        ));
                    }
                }
