use std::io::Read;
use crate::ast::{BinOp, Expr, Pattern, Stmt, TypeAnnot, LambdaBody};
use super::{write_u64_le, read_u64_le};

pub fn write_type_annot(data: &mut Vec<u8>, t: &TypeAnnot) {
    match t {
        ...
        TypeAnnot::Generic(name) => {
            ...
            TypeAnnot::Function { param_types, return_type } => {
                write_u64_le(data, 9);
                write_u64_le(data, param_types.len() as u64);
                for pt in param_types {
                    write_type_annot(data, pt);
                }
...
pub fn read_type_annot(cursor: &mut std::io::Cursor<&[u8]>) -> Option<TypeAnnot> {
    ...
    match expr {
        ...
        Expr::Lambda { params, return_type, body, .. } => {
            ...
            for (pname, pty) in params {
                ...
                write_type_annot(data, pty);
            }
            let ret_ty = return_type.as_ref().unwrap_or(&TypeAnnot::Void);
            write_type_annot(data, ret_ty);
            match body {
                LambdaBody::Expr(e) => { write_u64_le(data, 0); write_expr(data, e); }
                LambdaBody::Block(stmts) => {
                    write_u64_le(data, 1);
                    write_u64_le(data, stmts.len() as u64);
                    for s in stmts {
                        // Simplification: just write exprs inline
                        write_expr(data, &Expr::Number(0, 0, 0));
                    }
                }
            }
        }
...
pub fn read_stmt(cursor: &mut std::io::Cursor<&[u8]>) -> Option<Stmt> {
    Some(match read_u64_le(cursor)? {
        0 => {
            let name = read_string(cursor)?;
